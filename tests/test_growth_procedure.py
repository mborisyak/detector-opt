...
* the REWIND (``param_mix``) is inside the loop -- at a growth boundary it discards a fraction of the
  fit and resets the optimiser, which is visible as a jump in the training loss, and a run with
  ``param_mix = 0`` follows a different trajectory from the same seed;
...
def _config(param_mix):
  ...
  return {
    **base,
    ...
    'regressor': REGRESSOR,
    'training': {
      **SMALL_TRAINING, 'param_mix': param_mix, 'reveal': 'design'
    },
  }


def _run(param_mix, seed=17):
  """One design trained by the real procedure. Returns the per-epoch snapshot and the result."""
  config = _config(param_mix)
...
def test_the_rewind_is_inside_the_loop(rewound):
  """`param_mix` discards a fraction of the fit at every data addition and resets the optimiser, so the
  ...
  plain, _, _, _ = _run(param_mix=0.0)
  ...
  assert not same, 'param_mix = 0.25 and 0.0 produced the same trajectory, so the rewind is not in the loop'
