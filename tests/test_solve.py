...
def test_solve(seed, plot_root):
    ...
    v0 = 5 * jnp.array([0, 0, 1], dtype=jnp.float32) + jnp.stack(
        [
    ...
    for i in range(n):
        axes.plot(
            ...
            trajectory[:, i, 1],
            trajectory[:, i, 2],
            color=plt.cm.tab10(0) if q[i] < 0 else plt.cm.tab10(1),
            alpha=0.2 + 0.8 * float(m[i] / jnp.max(m)),
        )
    axes.set_xlabel("x")
    axes.set_ylabel("y")
    axes.set_zlabel("z")
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=n_cores) as executor:
        futures = [executor.submit(generator, seed=1, configurations=configs) for _ in range(n_trials)]
