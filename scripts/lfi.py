...
# The ragged-layout regressor/discriminator are incompatible with the padded
# (B, M, F) events from the new detector. Call sites are updated mechanically
# (loss/metric dict, detector.sample_events for ground truth); this script will
# not run end-to-end until the models are ported to the padded layout.
...
def optimize(seed, output, progress=True, restore=True, trace=None, report=None, **config):
    ...
    def loss_f(x, c, t, r_params, r_state):
        ...
        loss = jnp.mean(detector.loss(p, t)) + reg_coef * regressor.regularization()
    ...
    def step_design(design, x, c, gt, t, r_params, r_state, d_params, d_state, opt_state):
        loss, grad = jax.value_and_grad(loss_design, argnums=1)(x, c, gt, t, r_params, r_state, d_params, d_state)
    ...
    def sample(rng_seed, c):
        ...
        _ev = detector.sample_events(rng_seed, c_batch)
        gt, x, t = _ev["ground_truth"], _ev["X"], _ev["targets"]
    ...
    for i in status.epochs(starting_epoch, epochs):
        for j in status.training(steps):
            ...
            _ev = detector.sample_events(get_seed(), design_batch)
            ground_truth, measurements, target = (
                _ev["ground_truth"],
                _ev["X"],
                _ev["targets"],
            )
        ...
        for j in status.validation(validation_batches):
            ...
            ground_truth_pseudo, measurements_pseudo, design_batch_pseudo = sample_independent(get_seed(), get_seed(), design)
