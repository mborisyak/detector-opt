...
# The ragged-layout generator/discriminator/regressor are incompatible with the
# padded (B, M, F) events from the new detector. Call sites are updated
# mechanically; this script will not run end-to-end until the models are ported
# to the padded layout.
...
def generate(seed, output, progress=True, restore=True, **config):
    ...
    generator_def, generator_optimizer = (
        restored["generator"]["model"],
        restored["generator"]["optimizer"],
    )
    generator_parameters, generator_state = (
        restored["generator"]["parameters"],
        restored["generator"]["state"],
    )
    generator_optimizer_state = restored["generator"]["optimizer_state"]

    discriminator_def, discriminator_optimizer = (
        restored["discriminator"]["model"],
        restored["discriminator"]["optimizer"],
    )
    discriminator_parameters, discriminator_state = (
        restored["discriminator"]["parameters"],
        restored["discriminator"]["state"],
    )
    ...
    for i in status.epochs(starting_epoch, epochs):
        for j in status.training(steps):
            for k in range(substeps):
                design_batch_real = design[None, :] + design_eps * np_rng.normal(size=(batch, *detector.design_shape())).astype(
                    np.float32
                )
                _gt, measurements, _mask, target = detector(get_seed(), design_batch_real)

                design_batch_gen = design[None, :] + design_eps * np_rng.normal(size=(batch, *detector.design_shape())).astype(
                    np.float32
                )
            ...
            design_batch = design[None, :] + design_eps * np_rng.normal(size=(batch, *detector.design_shape())).astype(
                np.float32
            )
            _gt, measurements, _mask, target = detector(get_seed(), design_batch)
            rng, key_step = jax.random.split(rng, num=2)

            (
                generator_losses[i, j],
...
def report(seed, checkpoint, report, **config):
    ...
    rng, key_init = jax.random.split(rng, num=2)
    restored = detopt.utils.io.restore_state(checkpointer, detector, config, rngs=nnx.Rngs(key_init), restore=True)
    ...
    _gt, measurements, _mask, _ = detector((seed, seed), designs)
