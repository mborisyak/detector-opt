...
def optimize(
    ...
    def loss_f(x, c, t, r_params, r_state):
        ...
        loss = (
            jnp.mean(detector.loss(t, p))
            + reg_coef * regressor.regularization()
            + 1.0e-2 * jnp.mean(jnp.square(p))
        )
    ...
    def step_design(design, x, c, t, r_params, r_state, opt_state):
        (loss, _), grad = jax.value_and_grad(loss_f, argnums=1, has_aux=True)(
            x, c, t, r_params, r_state
        )
    ...
    for i in status.epochs(starting_epoch, epochs):
        for j in status.training(steps):
            for k in range(substeps):
                design_batch = design[None, :] + design_eps * np_rng.normal(
                    size=(batch, *detector.design_shape())
                ).astype(np.float32)
            ...
            design_batch = np.broadcast_to(
                design[None], shape=(batch, *detector.design_shape())
            )
            _, measurements, target = detector(
                seed=get_seed(), configurations=design_batch
            )
        ...
        if trace is not None:
            detopt.utils.io.save_design(
                detector, os.path.join(trace, f"design-{i:05d}.json"), design
            )
