...
def _nominal_design(cfg_path="config/bo.yaml"):
    ...
    return np.asarray([*nd["stations"], nd["angle"]], np.float32)  # flat [station_z(n), angle]
...
def main():
    ...
    det = StereoStrawDetector(max_dt=0.3, max_time=300.0, max_particles=25, data_dir=files[0])
    ...
    print(
        f"hits/event  sim {sim_mult.mean():.1f}  MC {mc_mult.mean():.1f}  (ratio {sim_mult.mean()/max(mc_mult.mean(),1):.2f})"
    )
    ...
    for lo, hi in [(8312, 8600), (8600, 8900), (8900, 9200), (9200, 9527)]:
        ...
        if len(m):
            print(
                f"  z[{lo},{hi}] ({(lo-8312)/100:.1f}-{(hi-8312)/100:.1f}m): median {np.median(m):.2f}  p90 {np.percentile(m,90):.1f}  N={len(m)}"
            )
    ...
    ax.hist(mc_mult, bins=30, alpha=0.5, color="k", label="MC")
    ax.hist(sim_mult, bins=30, alpha=0.5, color="C0", label="sim")
    ax.set(title=f"{MAGNET}: hits/event", xlabel="n hits")
    ax.legend()
    zlo, zhi = float(layers_z.min()), float(layers_z.max())
    zmar = 0.05 * (zhi - zlo)
    ax = fig.add_subplot(2, 3, 2)
    ax.hist(mc_z, bins=80, density=True, alpha=0.5, color="k", label="MC")
    ax.hist(sim_z, bins=80, density=True, alpha=0.5, color="C0", label="sim")
    ax.set(title="hit z occupancy", xlabel="z (cm)", xlim=(zlo - zmar, zhi + zmar))
    ax.legend()
