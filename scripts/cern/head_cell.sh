#!/bin/bash
# One (i, i+1) pair of the HEAD-ONLY transfer probe: the warm start tunes the final layer alone.
...
set -o pipefail
$PY -u scripts/probe_head_transfer.py --run "$RUN" --pair "$PAIR" --representation "$REPR" \
    --reveal "$REVEAL" --n-events 131072 --device cuda --output "$AFSOUT" \
    --n0 8192 --n-increment 8192 2>&1 | tee "$AFSOUT/run.log"
RC=$?
set +o pipefail
