#!/usr/bin/env python3
"""Does REGULARISATION close the train/validation gap that makes uninformative designs unconvergeable?
...
def _per_window(history):
  ...
  reports the gap at the LAST epoch (what the criterion saw) alongside its mean and max over the
  round's post-warmup epochs, which is the honest estimate of the bias floor, and the floor is
  what decides the `loss_precision` the design can be run at.
  """
  if len(history) == 0:
    return []
  ...
  for size in sorted(set(window.tolist())):
    ...
    rows.append({
      ...
      "diff_mean": float(diff[sel].mean()), "diff_max": float(diff[sel].max()),
    })
  return rows
...
def main():
  ...
  parser.add_argument("--checkpoint-dir", default="output/probe-dropout",
  ...
  parser.add_argument("--output", default="output/screen/dropout-probe.json")
  ...
  for setting in settings:
    ...
    row = {knob: setting, "status": status, "loss": loss, "slack": slack, "spent": spent, "per_window": per_window}
    if len(per_window) > 0:
      last = per_window[-1]
      row["diff"], row["err"], row["window"] = last["diff"], last["err"], last["window"]
      # The gap FLOOR: the largest `diff` seen at the largest window reached, i.e. not the lucky
      # epoch the stopping rule picked. It is what a lower `loss_precision` would have to clear.
      row["diff_last_round_max"] = last["diff_max"]
      row["diff_last_round_mean"] = last["diff_mean"]
    rows.append(row)
    if status == "converged":
      print(f"  {knob} {setting}: converged  level {loss:.4f}  slack {slack:.4g}  spent {spent}")
    for w in per_window[-4:]:
      print(f"      window {w['window']:>7d}  diff {w['diff']:.4f} (round mean {w['diff_mean']:.4f}, "
