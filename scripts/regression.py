...
# The ragged-layout BayesDeepSet regressor is incompatible with the padded
# (B, M, F) events from the new detector. Call sites below are updated
...
def regress(seed, output, progress=True, restore=True, report=None, **config):
    ...
    def loss_f(x, c, t, r_params, r_state):
        ...
        loss = jnp.mean(detector.loss(p, t))
