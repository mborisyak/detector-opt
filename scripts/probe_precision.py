...
# At MODULE level, not inside `main`: a function-local `import detopt.utils.config` rebinds the name
# `detopt` as a LOCAL of that function, so every other `detopt.*` in it raises UnboundLocalError
# unless the branch holding the import ran. It only ever ran with `--set`, so the no-override path was
# broken.
  ...
  * the named designs of `scripts/validate_inhibitor.py` -- the 2x2 factorial the chemistry implies
  ...
  if landscape is not None and len(str(landscape)) > 0:
    ...
    if designs.shape[1] != int(detector.design_dim()):
      ...
      raise SystemExit(
        f"probe_precision: {landscape} holds {designs.shape[1]}-dimensional designs but "
  ...
  if all(hasattr(detector, field) for field in ("inhibitor_bounds", "inhibitor_potency_bounds", "substrate_B_bounds")):
    ...
    for label, key in (("corner-2x2", "KNOWN-GOOD 2x2, two inhibitor doses"),
                       ("corner-2x2-strong", "KNOWN-GOOD 2x2, strong dose"), ("corner-uninformative",
                                                                              "uninformative (ceiling probe)")):
...
def replay_design(detector, config, results_path, seed):
  ...
  LARGELY OBSOLETE: `bo.py` now records the killing design itself, as a row with
  `status: "incomplete"`, so it can simply be read out of the file. This remains for results written
  before that existed.

  It is recoverable because the driver is deterministic given (root seed, iteration index): ONE
  `SeedSequence` split into a network branch and an iteration branch, the latter spawning one seed per
  iteration which drives both the proposal and the training. Replaying the recorded
  `(x_scaled, loss, loss_std)` triples against that same stream and proposing once more therefore
  reproduces it -- and replaying VERIFIES itself, because each recorded proposal must come back before
  it is appended. The mismatches are returned rather than raised, so a design is never silently wrong.
  """
  import detopt.bo
  ...
  optimiser = BayesianOptimizer(int(detector.design_dim()), gp=gp_cfg, ei=ei_cfg, kernel=kernel, n_init=n_init)
  # The SAME split bo.py makes, in the same order: the network branch is spawned and discarded here
  # (it seeds the trainer, not the optimiser), so the iteration branch lands on the identical stream.
  _network_seq, iteration_seq = np.random.SeedSequence(int(seed)).spawn(2)
  mismatch = []
  recorded = detopt.utils.io.complete_results(recorded)
  for iteration, entry in enumerate(recorded):
    proposed = np.asarray(optimiser.propose(int(iteration_seq.spawn(1)[0].generate_state(1)[0])), np.float32)
...
def run_config_for(
  ...
  if device is not None:
    run["device"] = device
  # THE BUDGET IS A POWER OF TWO and therefore a multiple of `batch`. `Trainer._make_pools` splits it
  # as `round(budget * val_fraction)` validation rows and the rest train, so the smallest legal size is
  # the one whose TRAIN share still covers one full window: `budget >= iteration_limit / (1 -
  # val_fraction)`, rounded UP to a power of two. The validation share then covers
  # `val_iteration_limit = iteration_limit * val_fraction / (1 - val_fraction)` automatically. At the
  # campaign cell that is 2^18 = 262144 (train 196608, validation 65536, both multiples of `batch`).
  needed = int(training["iteration_limit"]) / (1.0 - float(training["val_fraction"]))
  training["budget"] = 1 << max(0, int(math.ceil(math.log2(needed))))
  ...
  # CAPACITY. The widths of the set blocks, as `[[w, w], [w, w]]`. A network that UNDERFITS reports a
  # near-zero train/validation gap and a poor level, and no amount of data or scheduling moves either --
  # widening is the only lever that does, so it has to be settable without editing a config.
  if features is not None:
    run["regressor"][regressor_name]["features"] = features
  # Drops WEIGHTS, each ensemble member with its own mask; the block's aggregation read-out is exempt.
  if dropconnect is not None:
    run["regressor"][regressor_name]["dropconnect"] = float(dropconnect)
  # DROPCONNECT drops WEIGHTS, each ensemble member getting its own mask, so unlike dropout it never
  # touches the inputs. `None` removes the key entirely rather than passing 0, because the model's own
...
def measure(detector, run, x_scaled, seed, *, plot_dir=None):
  ...
  print(
    f"  init |params|_1 = {init_checksum:.6f} over {n_parameters} parameters "
  ...
  try:
    result = trainer.train(x_scaled, int(seed), step=0, on_epoch=on_epoch)
    if result is None:
  ...
  # THE FULL PER-EPOCH CURVES, kept alongside the per-round collapse. The collapse has one point per
  # data addition, which cannot show what happens INSIDE a round -- and that is exactly where a rewind
  # acts: the loss jumps when the network is pulled back toward its initial state, then recovers over
  # the following epochs. Rounded to 6 decimals; ~800 kB for the longest run, which is the price of
  # being able to draw the mechanism rather than assert it.
  if len(history) > 0:
    row["per_epoch"] = {
      ...
      "train_sem": [round(float(v), 6) for v in np.asarray(history["train_sem_per_epoch"])],
      "val_sem": [round(float(v), 6) for v in np.asarray(history["val_sem_per_epoch"])],
...
def main():
  ...
  parser.add_argument(
    "--unranked", type=int, default=0, metavar="N",
  ...
  parser.add_argument(
    "--extra-ranks", type=int, nargs="*", default=[], metavar="RANK",
  ...
  parser.add_argument(
    "--replay", nargs="*", default=[], metavar="RESULTS.JSON:SEED",
  ...
  parser.add_argument(
    "--seeds", type=int, nargs="+", default=[1, 126382657],
    help="the protocol requires >= 2; the campaign's own seeds make it paired with a run"
  )
  parser.add_argument(
    "--weight-decay", type=float, nargs="+", default=[None],
    help="AdamW weight decay to sweep (default: whatever the config declares)"
  )
  parser.add_argument(
    "--iteration-limit", type=int, nargs="+", default=[None],
    help="per-design window cap AND epoch length (default: the config's)"
  )
  parser.add_argument(
    "--precision", type=float, default=None, help="override `training.loss_precision`. It also enters `is_plateaued` and the "
  ...
  parser.add_argument(
    "--n0", type=int, default=None, help="override `training.n0`; -1 means `= iteration_limit`, which removes the "
  ...
  parser.add_argument(
    "--n-increment", type=int, default=None, help="override `training.n_increment`, the events added per growth round"
  )
  parser.add_argument(
    "--warmup-epochs", type=int, default=None, help="override `training.warmup_epochs`, the unconditional epochs after every "
    "data addition before the stopping rule may act"
  )
  parser.add_argument(
    "--patience", type=int, default=None, help="override `training.patience`, the epochs the plateau test fits its slope over"
  )
  parser.add_argument(
    "--n-models", type=int, default=None, help="override the regressor's `n_models`. `1` is a ONE-MEMBER ensemble, i.e. the "
  ...
  parser.add_argument(
    "--features", default=None, metavar="JSON", help="override the regressor's block widths, e.g. '[[48,32],[32,48]]'. The "
    "capacity lever: an underfitting network shows a near-zero train/validation "
    "gap AND a poor level, which neither more data nor a different schedule can "
    "fix"
  )
  parser.add_argument("--p-dropout", type=float, default=None, help="override the regressor's `p_dropout` (0 = dropout off)")
  parser.add_argument(
    "--dropconnect", type=float, default=None, metavar="RATE",
    help="drop individual WEIGHTS at this rate while training, each ensemble member with its own "
    "mask. It never touches the inputs, the aggregates, or the map that emits the aggregation gate. "
    "Unset is a strict no-op"
  )
  parser.add_argument(
    "--reinit-on-grow", action="store_true", default=None,
  ...
  parser.add_argument(
    "--param-mix", type=float, default=None, metavar="LAMBDA",
  ...
  parser.add_argument(
    "--plot-per-epoch", type=int, nargs="+", default=[0], choices=[0, 1],
  ...
  parser.add_argument(
    "--resume", action="store_true", help="keep cells already in --output and skip them (a SLURM job that was killed "
  ...
  for spec in arguments.replay:
    ...
    try:
      ...
      print(
        f"replay {path} seed {seed}: {n_recorded} recorded proposals reproduced {status}; "
      ...
      print(f"replay {path} seed {seed} FAILED ({type(error).__name__}: {error}); continuing without it", flush=True)
  ...
  print(
    f"seeds {arguments.seeds} | weight_decay {arguments.weight_decay} | "
  ...
  for iteration_limit, weight_decay in itertools.product(arguments.iteration_limit, arguments.weight_decay):
    for design_name, (x_scaled, proxy_loss) in designs.items():
      for seed in arguments.seeds:
        for plot in arguments.plot_per_epoch:
          run = run_config_for(
            config, weight_decay=weight_decay, iteration_limit=iteration_limit, precision=arguments.precision,
          ...
          row.update({
            ...
            "seed": int(seed),
            "weight_decay": cell[2],
            "iteration_limit": cell[3],
            "plot_per_epoch": int(plot),
            ...
            "n_models": n_models_set,
            "p_dropout": p_dropout_set,
            "features": run["regressor"][regressor_name].get("features"),
            "dropconnect": run["regressor"][regressor_name].get("dropconnect"),
          ...
          print(
            f"  -> {row['status']}: diff={row.get('diff', float('nan')):.5f} "
  ...
  if len(measured) > 0:
    ...
    print(
      f"MAXIMUM slack over cells: {worst['slack']:.5f} at design={worst['design']} seed={worst['seed']} "
      f"weight_decay={worst['weight_decay']:g} iteration_limit={worst['iteration_limit']} "
      f"({worst['status']})"
    )
    print(f"hit the cap: {sorted(set(capped))}")
  print(
    "READ IT AS: a CONVERGED cell is CENSORED at its `loss_precision` -- it proves the design can "
