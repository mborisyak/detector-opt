...
class FullBudgetTrainer(Trainer):
  """Confirm a single design by full-budget, fixed-length training.
    ...
    **single-cycle cosine-decayed** learning rate (peak -> ~0). No data growing, no
    plateau / precision gate -- the only stop is ``max_epochs``. ``combine`` sees the
    design's true geometry, via the shared kernels, but the NETWORK is not told it
    (:meth:`reveals_design` is ``False``) -- this trainer is per-design, so the design
    is constant across every batch it draws.
  ...
  def default_reveal(self):
    """One network per design at the full budget, so the design is constant across the batch.
      Withheld by default; set ``training.reveal`` to override."""
    return 'none'
  ...
  def from_config(cls, detector, config, *, max_epochs, seed=0):
    ...
    return cls(
      detector, regressor_config=config["regressor"], optimizer_config=training["optimizer"], batch=training["batch"],
      budget=training["budget"], max_epochs=max_epochs, val_fraction=training.get("val_fraction", 0.25),
      eval_batch=training.get("eval_batch"), device=resolve_device(config.get("device")), seed=seed,
    )

  def __init__(
    self, detector, *, regressor_config: dict, optimizer_config: dict, batch: int, budget: int, max_epochs: int,
    val_fraction: float = 0.25, eval_batch: int | None = None, device=None, seed: int = 0,
  ):
    ...
    super().__init__(
      ...
      iteration_limit=train_budget, val_iteration_limit=val_budget, val_fraction=val_fraction, eval_batch=eval_batch,
  ...
  def train(self, design_scaled, seed, *, on_epoch=None) -> TrainResult:
    ...
    with plot_ctx as plot_pool:
      for _ in range(self.max_epochs):
        ...
        if on_epoch is not None:
          plot_pool.submit(
            on_epoch, self._snapshot(train_hist, val_hist, train_sem_hist, val_sem_hist, window_hist, train_count, val_count,
                                     ),
          )
    ...
    spent_train, spent_val = tp.current, vp.current
    return TrainResult(
