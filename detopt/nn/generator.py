...
def elbo(
    ...
    latent_sigma: jax.Array,
    sigma_reconstructed: float = 1.0,
...
class HyperVAE(VAE):
    def __init__(
        ...
        *intermediate, latent_dim = units
        ...
        self.encoder_body = [nnx.Linear(n_in, n_out, rngs=rngs) for n_in, n_out in zip(units_encoder[:-2], units_encoder[1:-1])]
        self.encoder_head = (
            nnx.Linear(units_encoder[-2], units_encoder[-1], rngs=rngs),
            nnx.Linear(units_encoder[-2], units_encoder[-1], rngs=rngs),
        )
        self.decoder_body = [nnx.Linear(n_in, n_out, rngs=rngs) for n_in, n_out in zip(units_decoder[:-2], units_decoder[1:-1])]
        ...
        *_, n_emb = design_embedding
        ...
        self.hyper_encoder = [
            (nnx.Linear(n_emb, n_out, rngs=rngs), nnx.Linear(n_emb, n_out, rngs=rngs)) for n_out in units_encoder[1:-1]
        ]

        self.hyper_decoder = [
            (nnx.Linear(n_emb, n_out, rngs=rngs), nnx.Linear(n_emb, n_out, rngs=rngs)) for n_out in units_decoder[1:-1]
        ]
        self.latent_dim = latent_dim
...
class DeepSetVAE(VAE):
    def __init__(
        ...
        design_shape: Sequence[int],
        target_shape: Sequence[int],
        encoder_features: Sequence[Sequence[int]],
