...
class DeepSet(Regressor):
  ...
  def combine(self, hit_idx, hit_tdc, mask, design):
    ...
    positions, angles, magnetic_strength = ... # design

    hit_normed_index = ... # hit_indx / [max stations = 4, max views = 4, max layers = 2, max straws = 300]

    # station_indx = jnp.where(mask, station_indx, 0)
