...
def uniform_to_normal_jax(xs, low, high):
    """JAX (differentiable, jittable) counterpart of :func:`uniform_to_normal`."""
    xs = jnp.asarray(xs)
    ...
    us = jnp.clip((xs - c) / d, eps - 1, 1 - eps)
    return jsp.erfinv(us) * SQRT_2


def normal_to_uniform_jax(xs, low, high):
    """JAX (differentiable, jittable) counterpart of :func:`normal_to_uniform`."""
    xs = jnp.asarray(xs)
    us = jsp.erf(xs * INV_SQRT_2)
    c = (low + high) / 2
    d = (high - low) / 2
    return us * d + c
