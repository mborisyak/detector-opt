...
from . import jax_gp
from .jax_gp import (
  GPHParams, GPState, expected_improvement, fit_gp, gp_factorize, gp_posterior, gp_predict, optimise_ei, rbf_kernel, refit_gp,
)
from .bayesian_optimization import BayesianOptimizer
from .kernels import ARDRBF, IndependentSortingRBF, NormalisedInvariantRBF, PermutationInvariantRBF, SortingRBF
...
__kernels__: dict[str, type] = {
  ...
  # Sorting again, but each block ordered by ITS OWN values instead of by a shared key block, so
  # the invariance is the PRODUCT group rather than the diagonal one. For the MNIST window, whose
  # design names two opposite corners: `(x1, x2)` and `(y1, y2)` are unordered pairs that swap
  # INDEPENDENTLY, and this canonicalises every design to `(left, right, top, bottom)`. See
  # IndependentSortingRBF, whose docstring states the contrast with `sorting-rbf` in full.
  "independent-sorting-rbf": IndependentSortingRBF,
...
def kernel_from_config(config, detector, gp):
  ...
  if name == "ard-rbf":
    ...
    raise ValueError(
      f"`{name}` needs `exchangeable: <n>`, the number of interchangeable "
      f"design elements (e.g. n_experiments)"
    )
  elif name == "sorting-rbf":
    layout = {"sort_blocks": _exchangeable_blocks(detector, exchangeable), "key": arguments.pop("key", -1)}
  elif name == "independent-sorting-rbf":
    # Same index tuples as `sorting-rbf`, and deliberately so: the blocks are identical, only
    # what the kernel does with them differs. No `key` -- there is no block the others follow.
  ...
  return clazz(
    d=int(detector.design_dim()), **layout, constant_value=float(np.exp(amp_low + amp_high)),
    constant_value_bounds=(float(np.exp(2.0 * amp_low)), float(np.exp(2.0 * amp_high))),
    length_scale=float(np.exp(0.5 * (ls_low + ls_high))), length_scale_bounds=(float(np.exp(ls_low)), float(np.exp(ls_high))),
  )
